...
namespace DelegateExercises {
    ...
    public class Person {
        public delegate void Formatter(String FirstName, String LastName);
        ...
        public static void Main(string[] args) {
            ...
            while (true) {
                ...
                switch (input.ToUpper()) {
                    case "1":
                        people.ForEach(p => p.Print((F, L) => PrintFullNameLastNameFirst(F, L)));
                        break;
                    case "2":
                        people.ForEach(p => p.Print((F,L) => PrintFullNameAllCaps(F, L)));
                        break;
                    case "3":
                        people.ForEach(p => p.Print((F, L) => PrintFullNameLowerCase(F, L)));
                        break;
                    case "4":
                        people.ForEach(p => p.Print((F, L) => PrintShortName(F, L)));
                        break;
                    case "Q": return;
                    default:
